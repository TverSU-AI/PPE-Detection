...
from processing.image_processing import pack_json
import torch
import os
...
class DetectionLoader():
    ...
    def image_detection(self):
        ...
        self.det = (orig_imgs, im_names, boxes,
    ...
    def image_postprocess(self):
        with torch.no_grad():
            ...
            self.pose = (inps, orig_img, im_name, boxes,
                         scores, ids, cropped_boxes)
...
class DataWriter():
    ...
    def save(self, boxes, scores, ids, hm_data, cropped_boxes, orig_img, im_name):
        self.item = (boxes, scores, ids, hm_data,
...
class SingleImageAlphaPose():
        ...
        try:
            ...
            with torch.no_grad():
                (inps, orig_img, im_name, boxes, scores, ids,
                 cropped_boxes) = self.det_loader.process(im_name, image).read()
                if orig_img is None:
                ...
                if boxes is None or boxes.nelement() == 0:
                    ...
                    self.writer.save(None, None, None, None,
                    ...
                    if self.args.profile:
                        ...
                        runtime_profile['pt'].append(pose_time)
                    hm = hm.cpu()
                    self.writer.save(boxes, scores, ids, hm,
                                     cropped_boxes, orig_img, im_name)
                    pose = self.writer.start()
                    if self.args.profile:
                        ckpt_time, post_time = getTime(ckpt_time)
    ...
    def vis(self, image, pose):
        if pose is not None:
            image = self.writer.vis_frame(
                image, pose, self.writer.opt, self.writer.vis_thres)
        return image

    def writeJson(self, final_result, outputpath, form='coco', for_eval=False, outputfile='alphapose-results.json'):
        from alphapose.utils.pPose_nms import write_json
        write_json(final_result, outputpath, form=form,
                   for_eval=for_eval, outputfile=outputfile)
        print("Results have been written to json.")


def prepare_pose(all_results, img, for_eval=False, form=None):
    json_results = []
    json_results_cmu = {}
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            result = {}
            if for_eval:
                result['image_id'] = int(os.path.basename(
                    im_name).split('.')[0].split('_')[-1])
            else:
            ...
            if 'box' in human.keys():
                ...
                result['box'] = get_box(result['keypoints'], height, width)
...
class PoseEstimator:
    def __init__(self, args):
        ...
        self.estimator = SingleImageAlphaPose(args, cfg)
        self.uid = 0

    def process(self, image: MatLike, image_name: str | None = None) -> tuple[dict, MatLike]:
        if image_name is None:
            image_name = f"{self.uid}.jpg"
            self.uid += 1

        pose = self.estimator.process(image_name, image)

        packed_json = self.__get_packed_pose_json(pose)
        posed_image = self.__get_pos_visualisation(pose)

        return packed_json, posed_image

    def __get_pos_visualisation(self, pose):
        image = self.estimator.getImg()

        if pose["result"] == []:
            return self.estimator.vis(image, None)
        else:
            return self.estimator.vis(image, pose)

    def __get_packed_pose_json(self, pose):
        image = self.estimator.getImg()
        return pack_json(image, prepare_pose([pose], image))
